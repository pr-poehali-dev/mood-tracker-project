...
const Techniques = () => {
  ...
  const categories = ['Все', ...Array.from(new Set(techniques.map((t) => t.category)))];
      ...
      <Card className="border-0 shadow-xl bg-card/80 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            База техник самопомощи
          </CardTitle>
          <CardDescription>
            Выберите категорию и изучите пошаговые инструкции
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          >
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                >
                  <Icon
                    name={favorites.includes(technique.id) ? 'Star' : 'Star'}
                    size={20}
                    className={favorites.includes(technique.id) ? 'fill-yellow-500 text-yellow-500' : ''}
                  />
                </Button>
              </div>
